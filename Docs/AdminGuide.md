...
- **Melee attack target:** blocks caused *exclusively* by the creature (or player) the attacker is actively meleeing are accepted with **no budget charge** — sticky melee deliberately glues the attacker to the target's cylinder, so that contact is constant and expected for the whole fight. This covers both PvE melee inside packs and PvP chases. Bounded: only the single current attack target qualifies, the environment probe still rejects any wall involvement, and speed/avg checks validate every accepted packet. Third-party *pure-player* body-blockers take the tight player path below.
- **Mob-only and mixed player+mob blocks:** a leaky budget measured in **seconds of overlap** (3 s capacity, 0.6/s refill; each blocked packet charges real elapsed time, capped 0.25 s/packet), so it is packet-rate independent — a fast client at 30 packets/s no longer burns the budget in under a second. Mixed blocks route here deliberately: group hunting constantly produces blocked packets containing both a mob and a fellow player, and routing those to the strict player rule rubber-banded fellowships fighting around packs. Pack crossings and melee combat inside packs stay within budget indefinitely (intermittent contact refills faster than it drains); a client that deletes mobs and stands inside them is enforced after ~7.5 s of continuous overlap, with geometry, speed, and average-speed checks still applying to every accepted packet.
- **Blocks caused purely by other players** (no mobs involved, not your melee target): a deliberately tiny time budget (0.3 s capacity, 0.15/s refill, 0.1 s max charge/packet). Incidental brushes in a crowd are absorbed and refill within a couple of seconds, but a deliberate body-blocker standing in the path drains it in ~1/3 s of continuous contact and blocks solidly from then on — PK body-blocking remains effective while crowd desync and one-shot traps do not rubber-band.
