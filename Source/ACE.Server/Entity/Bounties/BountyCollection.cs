...
{
    /// <summary>
    /// In-memory cache of a player's active BountyContract WorldObjects, keyed by target GUID.
    /// </summary>
    public class BountyCollection
    {
        ...
        public void CheckContracts(Player owner)
        {
            foreach (var kvp in _contracts.ToList())
            {
                ...
                // Remove contracts that are in invalid states
                if (contract.IsInvalidContract)
                {
                    _contracts.Remove(kvp.Key);
                    owner.TryConsumeFromInventoryWithNetworking(contract);
                    continue;
                }

                // Expire contracts that have passed their expiration time
                if (contract.IsBountyExpired && !contract.IsExpiredState)
                {
                    contract.SetState(BountyContract.BountyContractState.Expired, owner);
