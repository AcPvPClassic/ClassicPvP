    ...
    public static class DefaultPropertyManager
    {
        ...
        public static readonly ReadOnlyDictionary<string, Property<bool>> DefaultBooleanProperties =
            DictOf(
                ...
                ("movement_collision_grace", new Property<bool>(true, "master switch for ALL collision-grace leniency: mob pack pass-through budget, player-brush budget, melee sticky-target exemption, and terrain/hill shortfall grace. Set false to revert blocked movement to stock always-rubber-band enforcement. Does NOT affect any detection or scoring checks (speed, geometry, doors, scripts) — those are governed by their own enforce_* properties")),
                ("enforce_player_dynamic_collision", new Property<bool>(true, "whether mobs and OTHER PLAYERS block player movement. Default true. Set false to let players phase through creatures and each other (a move blocked purely by dynamic objects, with no wall involvement, is accepted with no rubber-band). Walls, static objects, and the speed checks still enforce, so short-distance teleport/wall-clip is still prevented — keep enforce_player_movement on. Independent of movement_collision_grace")),
        ...
        public static readonly ReadOnlyDictionary<string, Property<double>> DefaultDoubleProperties =
            DictOf(
                ...
                ("movement_avg_strafe_ceiling_15s", new Property<double>(1.30, "anti-cheat: max sustained allowance for STRAFE segments on the 15-second window, as a flat multiplier over 4.0 x run rate (NOT stacked on movement_avg_ceiling_15s). Legit strafe-running is 1.248x forward speed; the default 1.30 adds a small margin. This is the cap a quickness hacker gets by faking the sidestep flag, so it is the tightest a determined speed cheat can sustain — lower toward 1.25 to tighten further (a real strafe-runner almost never averages 1.248x over a full 15 s, so this has large real margin), at a small false-positive risk. Do not set below 1.248")),
